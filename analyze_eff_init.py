...
with open('perf_log_efficientnet_initialized.txt', encoding='utf-8') as f:
...
n = len(fps_list)
print(f"=== DATA PENGUJIAN MODEL EFFICIENTNET (INITIALIZED) ===")
...
print(f"Std : {statistics.stdev(fps_list) if n > 1 else 'N/A'}")
print()
print(f"=== ML Kit Latency (ms) ===")
print(f"Min : {min(ml_list)}")
print(f"Max : {max(ml_list)}")
print(f"Avg : {sum(ml_list)/n:.2f}")
print(f"Std : {statistics.stdev(ml_list) if n > 1 else 'N/A'}")
print()
...
print(f"Std : {statistics.stdev(tot_list) if n > 1 else 'N/A'}")
print()
